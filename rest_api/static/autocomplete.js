...
function autocomplete(inp, get_items_function) {
  
  var currentFocus;
  
  // execute something, when input value changes
  inp.addEventListener("input", function(e) {
      ...
      matching_items = get_items_function(val)

      var c_id
      for (c_id in matching_items) {

        // get information from dict
        var item = matching_items[c_id]
        var name = item[0]
        var firstname = item[1]
        var club = item[2]
        var kind = item[3]
        ...
        if (kind == "name_match") {
          ...
          b.innerHTML += firstname.substr(val.length);
          b.innerHTML += " (" + club + ")";
        }

        /*insert a input field that will hold the current array item's value:*/
        b.innerHTML += "<input type='hidden' value='" + c_id + "'>";
